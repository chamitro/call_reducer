...
library BytesDeserializer {
   ...
   */
  function slice32(bytes b, uint offset)  returns (bytes32) {
    bytes32 out;
   ...
   */
  function sliceAddress(bytes b, uint offset)  returns (address) {
    bytes32 out;
   ...
   */
  function slice16(bytes b, uint offset)  returns (bytes16) {
    bytes16 out;
   ...
   */
  function slice4(bytes b, uint offset)  returns (bytes4) {
    bytes4 out;
   ...
   */
  function slice2(bytes b, uint offset)  returns (bytes2) {
    bytes2 out;
 ...
 * It's also recommended that you define s in the contract, like ROLE_ADMIN below,
 *  to avoid typos.
 */
contract RBAC {
  ...
  /**
   * A  role name for indicating admins.
   */
