#!/bin/bash
# Property check: slither must report "should emit an event for" exactly 3 time(s).
...
count=$(slither "$source_file" 2>&1 >/dev/null | grep -o "should emit an event for" | wc -l | xargs)
