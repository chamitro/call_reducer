...
class RemovalListener2(SolidityListener):
        ...
        if any(node.name == identifier for node in self.nodes_to_remove):
            ...
            if ctx.inheritanceSpecifier():
                ...
                for i, spec in enumerate(inheritance_specifiers):
                    ...
                    if any(node.name == spec_text for node in self.nodes_to_remove):
                ...
                if updated_inheritance:
                    ...
                    updated_inheritance_text = ", ".join(parent.name for parent in sorted_inheritance)
    ...
    def exitSourceUnit(self, ctx: SolidityParser.SourceUnitContext):
        for identifier in self.temp_graph.nodes:
            parents = list(self.temp_graph.predecessors(identifier))
            if parents:
                sorted_parents = self.sort_inheritance(parents)
                new_inheritance_specifiers = ", ".join(parent.name for parent in sorted_parents)
    ...
    def sort_inheritance(self, specifiers):
        ...
        try:
            ...
            # Handle cycles by returning an empty list or raising an error
...
class Interesting():
    ...
    def __call__(self, nodes, config_id):
        if self.mode == 'functions':
            nodes_to_remove = [n for n in self.graph.nodes() if n.node_type == 'function' and n not in nodes]
            fr_nodes = frozenset(nodes)
            if fr_nodes in self.cache:
                return self.cache.get(fr_nodes)
            if not nodes_to_remove:
                return picire.Outcome.FAIL
            new_content, updated_graph = self.test_removing_functions(nodes_to_remove, self.content)
...
#                self.graph = updated_graph
        ...
        if self.mode == 'contracts':
            nodes_to_remove = [n for n in self.graph.nodes() if n.node_type == 'contract' and n not in nodes]
            ...
            if not nodes_to_remove:
                return picire.Outcome.FAIL
            new_content, updated_graph = self.test_removing_contracts(nodes_to_remove, self.content)
            if new_content is not None:
                self.content = new_content
#                self.graph = updated_graph
                self.update_file(new_content)
    ...
    def test_removing_functions(self, nodes_to_remove, content):
        ...
        if modified_slither_output:
            ...
            if compare_slither_outputs(self.original_findings, modified_findings, consider_findings):
                ...
                updated_graph = self.update_graph(nodes_to_remove, self.graph.copy())
                return modified_content, updated_graph
            else:
                os.remove(temp_file_path)
                return None, None
        else:
    ...
    def test_removing_contracts(self, nodes_to_remove, content):
        ...
        if modified_slither_output:
            ...
            if compare_slither_outputs(self.original_findings, modified_findings, consider_findings):
                print("Keep property: Writing modified content to temp file")
                os.remove(temp_file_path)
                return modified_content, temp_graph
            else:
                os.remove(temp_file_path)
                return None, None
        else:
    ...
    def update_graph(self, nodes_to_remove, graph=None):
        graph = graph or self.graph
        for node in nodes_to_remove:
            if graph.has_node(node):
                # Remove edges connected to the node
                print(f"exei node",node)
...
def main():
    ...
    # Reload the graph with the modified content
#    graph = build_graph_from_file(sol_file_path)
