...
BADCC2=("docker run --rm -v $(pwd):/work gcc-4.9 gcc -m32 -O2 /work/small.c -o /work/t" "docker run --rm -v $(pwd):/work gcc-4.9 gcc -m64 -O2 /work/small.c -o /work/t")
BADCC3=() # wrong results
GOODCC=("docker run --rm -v $(pwd):/work gcc-4.9 gcc -m32 -O3 /work/small.c -o /work/t" "docker run --rm -v $(pwd):/work gcc-4.9 gcc -m64 -O3 /work/small.c -o /work/t")
TIMEOUTCC=10
TIMEOUTEXE=2
TIMEOUTCCOMP=10
CFILE=$(pwd)/small.c
CFLAG="-o t"
CLANGFC="gcc -O0 -Wall -fwrapv -ftrapv -fsanitize=undefined"
...
if
  gcc -pedantic -Wall -Wsystem-headers -O0 -c $CFILE  >out.txt 2>&1 &&\
...
for cc in "${GOODCC[@]}" ; do
    ...
    timeout -s 9 $TIMEOUTCC $cc $CFLAG >& /dev/null
    ret=$?
    if [ $ret != 0 ] ; then
      echo "exit 6"
      exit 1
