...
BADCC=("sudo docker run --rm -v $(pwd):/workspace gcc-4.8.2 gcc -m32 -Os /workspace/small.c -o /work/t")
GOODCC=("sudo docker run --rm -v $(pwd):/workspace gcc-4.8.2 gcc -m32 -O0 /workspace/small.c -o /work/t")
TIMEOUT=10
CFILE=$(pwd)/small.c
CFLAG="-o t"
CLANGFC="gcc -m64 -O0 -Wall -fwrapv -ftrapv -fsanitize=undefined"
...
for cc in "${GOODCC[@]}" ; do
    ...
    # compile
    timeout -s 9 $TIMEOUT $cc $CFLAG >&/dev/null
    ret=$?
    if [ $ret != 0 ] ; then
  echo "exit 5"
  docker ps --filter "ancestor=gcc-4.8.2" --format "{{.ID}}" | xargs -r docker kill
