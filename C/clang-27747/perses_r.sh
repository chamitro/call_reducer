#!/bin/bash
BADCC1=("docker run --rm -v $(pwd):/work clang-3.6.0 clang -O1 /work/small.c -o /work/t")
BADCC2=()
BADCC3=()
MODE=("-m32" "-m64")
...
readonly GOODCC=("docker run --rm -v $(pwd):/work clang-3.5.0 clang -m32 -O1 /work/small.c -o /work/t")
...
for cc in "${BADCC1[@]}" ; do
  for mode in "${MODE[@]}" ; do
    ...
    # compile
    (timeout -s 9 $TIMEOUTCC $cc $CFLAG $mode >out2.txt 2>&1) >& /dev/null
