...
BADCC1=()
BADCC2=()
BADCC3=("docker run --rm -v $(pwd):/work clang-3.8.0 clang -Os /work/small.c -o /work/t")
MODE=("-m32")
...
readonly GOODCC=("gcc -O0" "docker run --rm -v $(pwd):/workspace clang-3.7.0 clang -m32 -O1 /workspace/small.c -o /workspace/t")
...
readonly CFILE=$(pwd)/small.c
readonly CFLAG="-o t"
readonly CLANGFC="clang -w -m64 -O0 -Wall -fwrapv -ftrapv -fsanitize=undefined,address"
readonly CLANG_MEM_SANITIZER="clang -w -O0 -m64 -fPIE -pie -fsanitize=memory"
...
for ((i=0; i < ${#GOODCC[@]} ; ++i )) ; do
  ...
  if [ $i != 1 ] ; then
    ...
    timeout -s 9 $TIMEOUTCC $cc >& /dev/null
  fi
...
for cc in "${BADCC3[@]}" ; do
  for mode in "${MODE[@]}" ; do
    ...
    # compile
    timeout -s 9 $TIMEOUTCC $cc $mode >& /dev/null
    ret=$?
    if [ $ret != 0 ] ; then
      echo "exit 15"
      exit 1
