...
BADCC=("sudo docker run --rm -v small_c_removal_reduction.c:/usr/gcc/small.c theosotr/gcc-4.8.2 gcc -O3 /usr/gcc/small.c")
GOODCC=("ccomp -fall")
TIMEOUT=90
CFILE=small_c_removal_reduction.c
echo $CFILE
CFLAG="-o t"
CLANGFC="gcc -O0 -Wall -fwrapv -ftrapv -fsanitize=undefined,address"
...
for cc in "${BADCC[@]}" ; do
    timeout -s 9 $TIMEOUT $cc $CFLAG > out.txt 2>&1
